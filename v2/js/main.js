...
/* Inativar funções na ficha técnica */
document.querySelectorAll("p").forEach(p => {
    if (p.textContent.includes("Coordenação de Produto e Soluções Educacionais:") &&
        p.textContent.includes("Telma Lanini")) {
        p.style.display = "none";
    }
});
document.querySelectorAll("p").forEach(p => {
    if (p.textContent.includes("Gerente de Audiovisual:") &&
        p.textContent.includes("Gisele Lima")) {
        p.style.display = "none";
    }
});
